...
export const updateListing = async (req, res) => {
  try {
    ...
    if (
      ...
      existingListing.createdBy.toString() !== req.agent._id.toString()
    ) {
      return res.status(403).json(
        handleForbidden('Not authorized', 'Not authorized to modify this listing')
      );
    }
...
export const deleteListing = async (req, res) => {
  try {
    ...
    // Ensure the requesting agent is the owner
    if (
      !listingToDelete.createdBy ||
      listingToDelete.createdBy.toString() !== req.agent._id.toString()
    ) {
      return res.status(403).json(
        handleForbidden('Not authorized', 'Not authorized to delete this listing')
      );
    }
